  ...
  return (
    <DashboardLayout user={user}>
      <div className="space-y-6">
        <h1 className="text-3xl font-bold text-[var(--text)]">Pipeline Événements</h1>
            ...
            return (
              <div key={col.key} className="min-w-[280px] flex-shrink-0">
                <div className="flex items-center gap-2 mb-3 px-1">
                  ...
                  <span className="text-xs text-[var(--text-muted)] bg-[var(--surface)] px-2 py-0.5 rounded-full">{colEvents.length}</span>
                </div>
                <div className="space-y-3">
                    ...
                    return (
                      <Card key={evt._id}>
                        <div className="space-y-2">
                          <h4 className="text-sm font-semibold text-[var(--text)]">{evt.name || 'Événement'}</h4>
                          {(evt.dateStart || evt.date) && (
                            <p className="text-xs text-[var(--text-muted)]">{new Date(evt.dateStart || evt.date!).toLocaleDateString('fr-CA')}</p>
                          )}
                          {evt.organizerName && <p className="text-xs text-[var(--text-muted)]">{evt.organizerName}</p>}
                  ...
                  {colEvents.length === 0 && (
                    <div className="text-center py-8 text-[var(--text-muted)] text-xs border border-dashed border-[var(--border)] rounded-lg">Vide</div>
                  )}
                </div>
              </div>
            );
