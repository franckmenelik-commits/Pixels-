  ...
  return (
    <DashboardLayout user={user}>
      <div className="space-y-6 max-w-4xl">
        <button onClick={() => router.back()} className="text-[#8E8E9A] hover:text-[#F0F0F0] text-sm">&larr; Retour</button>

        {!mission ? (
          <p className="text-[#8E8E9A]">Chargement de la mission...</p>
        ) : (
          <>
            <div className="flex items-start justify-between">
              <div>
                <h1 className="text-3xl font-bold text-[#F0F0F0]">{mission.eventName || 'Mission'}</h1>
                {mission.date && <p className="text-[#8E8E9A] mt-1">{new Date(mission.date).toLocaleDateString('fr-CA')}</p>}
              </div>
              <Badge variant={statusVariant(mission.status)}>{mission.status || 'nouveau'}</Badge>
            </div>

            <Card>
              <h2 className="text-xl font-semibold text-[#00D2FF] mb-3">Informations</h2>
              <div className="grid grid-cols-2 gap-4 text-sm">
                {mission.venue && <div><span className="text-[#8E8E9A]">Lieu:</span> <span className="text-[#F0F0F0]">{mission.venue}</span></div>}
                {mission.description && <div className="col-span-2"><span className="text-[#8E8E9A]">Description:</span> <span className="text-[#F0F0F0]">{mission.description}</span></div>}
              </div>
            </Card>

            {/* Roster */}
            <Card>
              <h2 className="text-xl font-semibold text-[#00D2FF] mb-3">Formation</h2>
              {(!mission.roster || mission.roster.length === 0) ? (
                <p className="text-[#8E8E9A] text-sm">Aucun musicien assigné.</p>
              ) : (
                <div className="space-y-2">
                  {mission.roster.map((slot, i) => (
                    <div key={i} className="flex justify-between items-center p-3 rounded-lg bg-[#0F0F1A]/50">
                      <div>
                        <span className="text-[#F0F0F0]">{slot.artistName || 'Non assigné'}</span>
                        {slot.instrument && <span className="text-[#8E8E9A] ml-2">— {slot.instrument}</span>}
                      </div>
            ...
            {mission.financials && (
              <Card>
                <h2 className="text-xl font-semibold text-[#00D2FF] mb-3">Finances</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: 'Total', value: mission.financials.total, color: '#F0F0F0' },
                    { label: 'Part musiciens', value: mission.financials.musicianShare, color: '#6C5CE7' },
                    { label: 'Logistique', value: mission.financials.logistics, color: '#00D2FF' },
                    { label: 'Réserve', value: mission.financials.reserve, color: '#FF6B6B' },
                  ].map(f => (
                    <div key={f.label} className="text-center">
                      <div className="text-2xl font-bold" style={{ color: f.color }}>${f.value || 0}</div>
                      <div className="text-[#8E8E9A] text-sm">{f.label}</div>
                    </div>
                  ))}
                </div>
              </Card>
            ...
            {(user.role === 'admin' || user.role === 'operator') && (
              <Card>
                <h2 className="text-xl font-semibold text-[#00D2FF] mb-3">Actions</h2>
